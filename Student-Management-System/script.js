...
function renderTable(data) {
  let html = `<table>
    <tr><th>Name</th><th>Email</th><th>Gender</th><th>Class</th><th>Marks</th><th>Passing</th></tr>`;
  data.forEach(s => {
    html += `<tr>
      <td><img src="${s.img_src}" class="profile-pic" alt="${s.first_name}" /><span>${s.first_name} ${s.last_name}</span></td>
      <td>${s.email}</td>
      <td>${s.gender}</td>
      <td>${s.class}</td>
      <td>${s.marks}</td>
...
function sortByMarks() {
  renderTable([...students].sort((a, b) => a.marks - b.marks));
}

function showPassing() {
  renderTable(students.filter(s => s.passing));
}

function sortByClass() {
  renderTable([...students].sort((a, b) => a.class - b.class));
}
...
function sortByGender() {
  ...
  document.getElementById("tableContainer").innerHTML = `
    <h3>Female Students</h3>${generateGenderTable(females)}
    <h3>Male Students</h3>${generateGenderTable(males)}
  `;
}

function generateGenderTable(data) {
  let html = `<table>
    <tr><th>Name</th><th>Email</th><th>Gender</th><th>Class</th><th>Marks</th><th>Passing</th></tr>`;
  data.forEach(s => {
    html += `<tr>
